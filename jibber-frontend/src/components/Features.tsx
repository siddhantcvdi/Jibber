...
function Feature({
  ...
  return (
    ...
    >
      <div
        className={`absolute inset-0 bg-gradient-to-br opacity-0 group-hover:opacity-3 transition-opacity duration-400 ${color}`}
      ></div>

      <div
        className={`absolute top-0 left-0 right-0 h-1 ${color} transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-400 ease-in-out`}
      ></div>

      <div className="flex items-start">
        <div
          className={`p-4 bg-gradient-to-br from-[#eef0ff] to-[#f5f6ff] dark:from-[#2d2f6b]/70 dark:to-[#373a85]/70 rounded-2xl shadow-sm flex-shrink-0 group-hover:scale-105 transition-transform duration-300 ${isHovered ? 'ring-1 ring-opacity-40' : ''} ring-[#5e63f9]`}
        >
          {icon}
        </div>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12 relative z-10">
          ...
          <p className="text-muted-foreground text-lg max-w-3xl mx-auto mb-6">
            Powered by cutting-edge cryptographic protocols that ensure your
        ...
        <div className="mt-14 bg-background dark:bg-card rounded-2xl shadow-md border border-border p-6 max-w-6xl mx-auto relative cursor-pointer hover:shadow-lg transition-all duration-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative z-10">
            <div className="text-center p-3 hover:transform hover:scale-105 transition-transform duration-300">
              <div className="text-3xl font-bold mb-1 bg-clip-text text-transparent bg-gradient-to-r from-[#5e63f9] to-[#7c7fff]">
                256-bit
              </div>

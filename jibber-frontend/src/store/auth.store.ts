...
export const authStore = create<AuthState>((set, get) => ({
    ...
    registerUser: async (userData) => {
        ...
        try {
            ...
            const { data: startData } = await api.post(`${backendURL}/auth/register-start`, {
            ...
            const { data: finishData } = await api.post(`${backendURL}/auth/register-finish`, {
                username: userData.username,
                email: userData.email,
                registrationRecord
            });
    ...
    loginUser: async (userData: LoginData) => {
        ...
        try {
            ...
            const {data: startData } = await api.post(`${backendURL}/auth/login-start`, {
            ...
            if (!loginResult || !loginResult.finishLoginRequest) {
                throw new Error("Login Failed - Invalid credentials or missing finishLoginRequest");
            }

            const { finishLoginRequest } = loginResult;

            const { data: finishData } = await api.post(`${backendURL}/auth/login-finish`, {
                usernameOrEmail: userData.usernameOrEmail,
                finishLoginRequest
            });
